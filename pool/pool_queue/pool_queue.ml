...
let run_job
  ...
  let tags =
    ...
    CCOption.map_or ~default:(create_by_label database_label) (add_by_label database_label) tags
  in
...
let start () =
  let tags = Database.Logger.Tags.create_by_label Database.Pool.Root.label in
  ...
  let handle fcn database_label =
    try%lwt fcn (Database.label_ctx database_label) with
