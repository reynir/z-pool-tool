...
let location urlencoded db_ctx =
  Field.(Location |> show)
  |> CCList.pure
  ...
  >>= Pool_location.find db_ctx
;;

let default_lead_time_settings db_ctx =
  Lwt.both
...
let list req =
  ...
  let%lwt can_access_session_assistants =
    can_access_session_assistants db_ctx actor experiment_id
  in
  Lwt_result.ok
  @@
  ...
  | true ->
    ...
    in
    Time_window.query_by_experiment ~query db_ctx experiment_id >|> to_html
  | false ->
    let chronological =
    ...
    (match chronological with
     | true -> Session.query_by_experiment ~query db_ctx experiment_id >|> to_html
     | false ->
       Session.query_grouped_by_experiment ~query db_ctx experiment_id
...
let duplication_session_data req db_ctx =
  let open Session in
  let experiment_id = experiment_id req in
  let session_id = session_id req in
  let* experiment = Experiment.find db_ctx experiment_id in
  ...
  let* parent_session =
    ...
    |> CCOption.map_or ~default:(Lwt_result.return None) (fun id ->
      find db_ctx id >|+ CCOption.return)
  in
  let%lwt followups = find_follow_ups db_ctx session_id in
...
let create req =
  ...
  let result ({ Pool_context.user; _ } as context) =
    ...
    let* events =
      ...
      | false ->
        let* location = location urlencoded db_ctx in
...
let session_page db_ctx req context session experiment =
  ...
  | `Edit ->
    ...
    let%lwt available_tags =
      Tags.ParticipationTags.(
        find_available db_ctx (Session (Session.Id.to_common session_id)))
    in
    let%lwt experiment_participation_tags =
      Tags.ParticipationTags.(
        find_all db_ctx (Experiment (Experiment.Id.to_common experiment_id)))
    in
    let%lwt text_messages_enabled = Pool_context.Tenant.text_messages_enabled req in
  ...
  | `Close ->
    let%lwt assignments, custom_fields =
      Assignment.find_for_session_close_screen db_ctx session.Session.id
    in
    let%lwt participation_tags =
      Tags.ParticipationTags.(
        find_all db_ctx (Experiment (Experiment.Id.to_common experiment_id)))
    in
    let%lwt counters = Assignment.counters_of_session db_ctx session_id in
  ...
  | `Cancel ->
    let%lwt follow_ups = Session.find_follow_ups db_ctx session_id in
...
let time_window_page db_ctx req context time_window experiment =
  let open Utils.Lwt_result.Infix in
  let open Helpers.Guard in
  ...
  let current_tags () =
    let open Tags.ParticipationTags in
    find_all db_ctx (Session (Session.Id.to_common time_window_id))
  in
  let create_layout = create_layout req context in
  function
  | `Edit ->
    ...
    let%lwt available_tags =
      Tags.ParticipationTags.(
        find_available db_ctx (Session (Session.Id.to_common time_window_id)))
    in
    let%lwt experiment_participation_tags =
      Tags.ParticipationTags.(
        find_all db_ctx (Experiment (Experiment.Id.to_common experiment_id)))
  ...
  | `Print ->
    let%lwt assignments =
      Assignment.(
        find_for_session_detail_screen ~query:default_query db_ctx time_window_id)
    in
    Page.Admin.Session.print
      ~view_contact_name
      ~view_contact_info
  ...
  let result context =
    ...
    (match Experiment.is_sessionless experiment with
     | false ->
       ...
       session_page db_ctx req context session experiment page
     | true ->
       let* time_window = Time_window.find db_ctx session_id in
       let* page =
         match page with
       ...
       in
       time_window_page db_ctx req context time_window experiment page)
    >|+ Sihl.Web.Response.of_html
  in
  Response.handle ~src req result
;;
...
let update_handler action req =
  ...
  let result ({ Pool_context.user; _ } as context) =
    ...
    let sessions_data db_ctx ({ Session.id; follow_up_to; _ } as session) =
      let%lwt follow_ups = Session.find_follow_ups db_ctx id in
      let* parent =
        match follow_up_to with
        | None -> Lwt_result.return None
        | Some parent_id -> parent_id |> Session.find db_ctx >|+ CCOption.return
      in
        ...
        (match Experiment.is_sessionless experiment with
         ...
         | false ->
           let* session, follow_ups, parent =
             Session.find db_ctx session_id >>= sessions_data db_ctx
           in
           let* location = location urlencoded db_ctx in
           let open CCResult.Infix in
           let open Update in
...
let enroll_from_main
  ...
  | false -> Lwt.return_ok ()
  | true ->
    let* follow_up_session = Session.find db_ctx follow_up_session_id in
    let%lwt assignments =
      Assignment.find_uncanceled_by_session db_ctx session.Session.id
    in
    assignments
...
let close_post req =
  ...
  let result ({ Pool_context.user; _ } as context) =
    ...
    let%lwt participation_tags =
      ...
      |> Lwt_list.fold_left_s
           (fun tags entity -> find_all db_ctx entity ||> CCList.append tags)
           []
    in
    let* events =
      assignments
      |> Lwt_list.map_s
           (fun ({ Assignment.no_show; participated; contact; _ } as assignment) ->
              ...
              let%lwt follow_ups =
                ...
                with
                | true -> find_follow_ups db_ctx assignment ||> CCOption.return
...
module DirectMessage = struct
  let assignments_from_requeset req db_ctx session_id =
    let open Assignment in
    Sihl.Web.Request.urlencoded_list Field.(array_key Assignment) req
    ||> CCList.map Id.of_string
    >|> find_multiple_by_session db_ctx session_id
    ||> function
  ...
  let modal_htmx req =
    ...
    let result context =
      ...
      let%lwt message_template =
        ...
        in
        Message_template.(
          find_by_label_and_language_to_send db_ctx ManualSessionMessage language)
      in
