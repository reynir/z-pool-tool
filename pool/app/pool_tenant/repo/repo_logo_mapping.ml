...
let insert_multiple = Lwt_list.iter_s (Sql.insert Database.(label_ctx Pool.Root.label))
let find_by_tenant = Sql.find Database.(label_ctx Pool.Root.label)
let find_all = Sql.find_all Database.(label_ctx Pool.Root.label)
let delete = Sql.delete Database.(label_ctx Pool.Root.label)
