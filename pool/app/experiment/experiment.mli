...
module Public : sig
  ...
  val contact_matches_filter : Database.ctx -> t -> Contact.t -> bool Lwt.t
...
module InvitationReset : sig
  ...
  val equal : t -> t -> bool
  val pp : Format.formatter -> t -> unit
  val find_by_experiment : Database.ctx -> Id.t -> t list Lwt.t
  val find_latest_by_experiment : Database.ctx -> Id.t -> t option Lwt.t
  val invitations_sent_since_last_reset : Database.ctx -> Id.t -> int Lwt.t
end
...
val contact_meets_criteria : Database.ctx -> t -> Contact.t -> bool Lwt.t
val find : Database.ctx -> Id.t -> (t, Pool_message.Error.t) result Lwt.t
...
val invitation_count : Database.ctx -> Id.t -> int Lwt.t
val session_count : Database.ctx -> Id.t -> int Lwt.t
...
val contact_is_enrolled : Database.ctx -> Id.t -> Contact.Id.t -> bool Lwt.t
...
val get_default_public_title : Database.ctx -> PublicTitle.t Lwt.t
...
val registration_possible : Database.ctx -> Id.t -> bool Lwt.t
...
val assignment_counts : Database.ctx -> Id.t -> assignment_counts Lwt.t
...
val invited_contacts_count : Database.ctx -> Id.t -> int Lwt.t
