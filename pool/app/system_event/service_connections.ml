...
let rerun_migrations_for_connection_issues () =
  ...
  Logs.debug ~src (fun m ->
    m ~tags:(Logger.Tags.create_by_label Pool.Root.label) "Check migrations for connection issues");
  let connect_and_migrate pool =
    Pool_database.connect_and_migrate pool
    >|- Pool_common.Utils.with_log_error ~tags:(Database.Logger.Tags.create_by_label pool)
    ||> Utils.ignore_res
  in
